...
/**
 * Базовый фрагмент для фрагментов со списком радиостанций.
 */
@AndroidEntryPoint
abstract class BaseRadioListFragmentAbstract : BaseContentFragmentAbstract() {
    companion object {
        private const val TAG = "BaseRadioListFragment"
    }

    // VIEW BINDING -> 1. Объявляем переменную. This property is only valid between onCreateView and onDestroyView
    private var binding: LayoutRadioStationListBaseBinding? = null

    private val viewModel by viewModels<BaseRadioListViewModel>()
    ...
    override fun onCreateView(
        ...
        binding = LayoutRadioStationListBaseBinding.inflate(inflater, container, false)
        // TOOLBAR
